// Since we are not updating or creating new emojis, I just fetched it once for efficiency (even though this operation is very small)
let correctAnswer;
let score = 0;
let timeoutId;
let playerName = "";
...
async function fetchNewEmoji(difficulty) {
  try {
    ...
    if (!res.ok) {
      throw new Error("Problem fetching Emoji.");
    }
